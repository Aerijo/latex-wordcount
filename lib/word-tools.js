...
function wordcount(filePath, {dirPath, merge = true, additionalOptions}) { // filePath must be absolute, dirPath is used for \input paths
  if (!dirPath) { dirPath = path.dirname(filePath); }

  let texdocCommand = `texcount -dir="${dirPath}"`;
  if (merge) { texdocCommand += " -merge"; }
  if (additionalOptions) { texdocCommand += (" " + additionalOptions.trim() + " "); }

  let additionalUserOptions = atom.config.get("latex-wordcount.additionalUserOptions");
  if (additionalUserOptions.length > 0) {
    texdocCommand += (" " + additionalUserOptions.trim() + " ");
  }
...
function wordcountAccurate(filePath) {
  ...
  let engine = "latex";
  if (atom.config.get("latex-wordcount.enableShellEscape")) { engine += " -shell-escape"; }
  // command should end as: latex wordcount.tex\csname ?\endcsname <file path>
...
function wordcountSelection(selectedText, dirPath) {
  ...
  tmp.file((err, path, fd, cleanupCallback) => {
    fs.writeFile(path, selectedText, (err) => {
      ...
      wordcount(path, {dirPath, merge: true});
    });
...
function foldSection(editor, point, maxDepthString) {
  let textRange = getSectionRange(editor, point, maxDepthString, false, false);

  if (!textRange || textRange.isEmpty()) { return; }

  if (!textRange.isSingleLine() && (textRange.end.row !== editor.getLastBufferRow())) { // make the folding prettier by ignoring the line the next section command is on.
    let oldRow = textRange.end.row;
    let newRow = oldRow - 1;
    let newRowLength = editor.lineTextForBufferRow(newRow).length;
    let endPoint = new Point(newRow, newRowLength);
    textRange.end = endPoint;
  }

  editor.foldBufferRange(textRange);
  editor.setCursorBufferPosition(textRange.end);
}

function getSectionRange(editor, point, maxDepthString, includeSectionCommand = true, continueIfNoStart = true) {
  /*
  ...
  */
  ...
  editor.backwardsScanInBufferRange(searchRegex, scanRange, ({match, range, stop}) => {
    ...
    if (maxDepth < levelTable[command]) { return; }
    ...
    if (includeSectionCommand) {
      ...
      startPoint = range.start.translate(new Point(0, match[0].length));
    }
  ...
  if (!matchFound) {
    if (!continueIfNoStart) { return 0; }
    notify.addWarning("Section level not found. Beginning from start of current file assuming `section` depth.");
    startPoint = new Point(0, 0);
    sectionLevel = levelTable.section;
  }
  ...
  if (!matchFound) {
    ...
    if (atom.config.get("latex-wordcount.verboseOutput")) { notify.addWarning("Next sectioning command not found, stopped at end of file"); }
  }
...
module.exports = { wordcount, wordcountAccurate, getRootFilePath, wordcountSelection, wordcountSection, foldSection };
